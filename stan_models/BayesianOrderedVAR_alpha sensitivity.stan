...
parameters {
  ...
  // Define the Cholesky factor for correlations and then scale.
  cholesky_factor_corr[K] L_corr;
  vector<lower=0>[K] Z_scale;
...
transformed parameters {
  ...
  matrix[K,K] L_Sigma = diag_pre_multiply(Z_scale, L_corr);
  ...
  for (s in 1:S) {
    ...
    for (t in start[s]:end[s]) {
          ...
          X_star[,t] = c + subject_intercept[,s] 
                         ...
                         + L_Sigma * X_star_innovation[,t]  // Transforms our standard normal (white noise) innovations to correlated innovations.
                         + time_of_day_effect[beep[t]];
    }}}
