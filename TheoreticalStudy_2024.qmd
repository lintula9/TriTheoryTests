...
Second, the covariance between two time points called cross covariance is
...
Notably, there also exists the dynamic factor model [@molenaar1985] which is a general case of a dynamic latent process. S-LMI differs in that it does not require stationarity to be estimable, the residuals (or 'errors' or 'innovations') are assumed to be independent of each other with diagonal covariance function (i.e., cross-sectional covariance and between time points covariance is diagonal) and that the cross-covariance between any two time points is symmetric as we will see below. s-LMI with 1 common factor decomposes the covariance as $$\Sigma=\Lambda\Lambda^T+\Omega$$where by definition of s-LMI $\Omega$ is assumed diagonal and and $\Lambda$ is a $K\times1$ column vector of 'factor loadings' assumed constant over time. We also need the cross covariance of the common factor. Let $\delta$ be the latent regression coefficient which links the common factor to itself at a previous time point such that $\eta_{t+1}=\delta\eta_t+\psi_t$, where $\psi_t$ is independent random term ('innovation', 'error', 'disturbance') with $E[\psi_t]=0$. Assuming standardized common factor such that $E[\eta]=0,\:Var(\eta)=1$ covariance of the common factor at two subsequent time points is

The s-LMI imposed $TK\times TK$ covariance matrix is $$
\begin{pmatrix}
   \Lambda\Lambda^T + \Omega & ... &  \Lambda\Lambda^T \prod\delta + \Omega_{\Delta}
\\ \vdots&\ddots&\vdots
\\ \Lambda\Lambda^T\prod \delta + \Omega_{\Delta}^T&...&\Lambda\Lambda^T(\prod\delta+Var(\psi_t)) + \Omega
\end{pmatrix}
$$

(Proof is in another file.)

The above cross-covariance matrix has only symmetric matrices as its blocks.
