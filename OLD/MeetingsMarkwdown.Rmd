...
```{r plot of within level VAR model, fig.width=7,fig.height=6,fig.cap="Within person VAR(1) model."}
Plot <- qgraph(VARparams, directed = T, diag = T, edge.labels = T)
```


## Longitudinal data, temporal analysis.
```{r, echo = FALSE, fig.width=10,fig.height=8,fig.cap="Example VAR(1) process."}
matplot(Sim1, type = "l", 
        ylab = "Within person symptom process", xlab = "Time", 
        main = "Three variable VAR(1) process.", family = "serif")
...
```{r, echo = FALSE}
round( cov( na.omit(simstime1) ), 2)
```
Covariance at random time 2 =  time 1 + 100, of 10000 samples.  
```{r, echo = FALSE}
round( cov( na.omit(simstime2) ), 2)
```
## Longitudinal data, temporal analysis.
...
- Is it generally discussed how AR processes predict symptomatology [e.g, ]
