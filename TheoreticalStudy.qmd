...
-   Add/integrate previous Rmd file of theoretical result as extension
    to this one (exact place TBD 30.1.).
...
matrices (or a precision matrix) and otherwise only re-scaling with a
scalar, multiplying by a diagonal matrix (since diagonal matrix commutes
with all matrices) is done for symmetric matrices.
...
and 0 regression coefficient for the latent variable). This is a fairly
simple, although less practically meaningful case. Scenarios when 0
across time points covariance are not as often observed arguably. This
...
Luckily, we have that the VAR(1) imposed covariance between any two
subsequent time points is the same. This means that $\delta_t$ must be a
constant since otherwise the s-LMI imposed covariance
$\Lambda\Lambda^T\delta + \Omega_{across}$ would not be the same between
two subsequent time points. $\Omega_{across}$ is diagonal and can only
compensate the diagonal to some extent - off-diagonal elements would not
be the same. This also leads to the constraint that $\Omega_{across}$ is
the same. More clearly put $$
\begin{align*}
\delta_1=\delta_2=...&=\delta
\\
\Omega_{across,1}=\Omega_{across,2}=...&=\Omega_{across}
\end{align*}
...
-   A direct observation from the null hyopothesis (1) and auxiliary
    null hypothesis (A1) equations shown above is that $A$ must always
    be symmetric for the common factor s-LMI model to be compatible with
...
always symmetric (since the diagonal matrix $\deltaI$ is symmetric, and
a covariance matrix multiplied by a diagonal matrix is also symmetric,
as is the sum of two symmetric square matrices) then $A$ must also be
symmetric in all cases for the null hypothesis (1) to hold. This
