...
data {
  ...
  matrix[N,p]        Y_case1;      // observed series, scaled to mean 0 and SD 1.
}
...
parameters {
  ...
  // Noise
  array[N]   vector[p]    epsilon;         // Noise, which is
                                           // _not_ contained in the Markov process.
...
transformed parameters {
  ...
  array[N]   vector[p] Y_pred;             // predicted observation
  ...
  for(t in 1:N)
    Y_pred[t] = X[t] + epsilon[t];
}

model {
  
  // Noise has an extremely strict prior
  for(t in 1:N)
    to_vector(epsilon[t]) ~ normal(0,0.01);
  
  // The latent process has weaker priors
  c                  ~ normal(0,0.5);
  ...
  for (t in 1:N)
    row(Y_case1, t) ~ normal(to_row_vector(Y_pred[t]), to_row_vector(residual_sd));
