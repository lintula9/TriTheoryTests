...
Before the analysis, a brief substantive consideration. We will assume
(*as Jaakko suggested*) that the symptom-network represented as the
VAR(1) model is a state. By this we mean that it is stationary. In
addition to stationarity, we also assume that there are no external
...
##### the above equations could be proven by induction to be true for delta\^(Dt). Other route maybe is to show that the ratio between covariances between two time points of distance delta_t and delta_t+1 is constant. Or show injectivity maybe. This is currently omitted.
