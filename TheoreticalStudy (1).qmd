...
so the covariance of VAR(1) is denoted as above from hereon. $\Gamma_t$
is a random $K\times1$ column vector of (serially) independent
innovations at time point $t$, with
\$E\[\\Gamma\]=0,\\text{Var}(\\Gamma=1)\$. $\Psi$ is covariance of the
...
substituting $A$ into the upper equation and then $A-\delta I$
...
VAR(1) does (*as Jaakko suggested*) restrain the analysis to a
...
Few more generalizations before we can equate the VAR(1) implied
covariance to the s-LMI implied covariance. We have that the VAR(1)
imposed covariance between any two subsequent time points is the same.
This also generalizes to the VAR(1) imposed covariance between any two
equidistant time points as they are $A^{\Delta t}\Sigma$. This means
that $\delta_t$ must be a constant since otherwise the s-LMI imposed
covariance $\Lambda\Lambda^T\delta + \Omega_{t,t+1}$ would not be the
same between two subsequent time points. $\Omega_{t,t+1}$ is diagonal
and so can only change the diagonal to some extent - off-diagonal
elements would not be the same. This also means that $\Omega_{t,t+1}$ is
the same for any $t$. More clearly put $$
\begin{align*}
...
This implies that the covariance between the common factor to itself at
any two time points must be
...
(1). (The above is true for all $\Delta t$.)

### Summary and implications 30.1.2024
