...
##### the above equations could be proven by induction to be true for delta\^(Dt). Other route maybe is to show that the ratio between covariances between two time points of distance delta_t and delta_t+1 is constant. This is currently omitted.
...
We can now resume to the working null hypothesis (1). Equating the
VAR(1) imposed covariances and s-LMI imposed covariances, as before, but
now using three subsequent time points we have

$$
\begin{cases}
&A\Sigma A^T+\Psi&=&\Lambda \Lambda^T + \Omega\\
...
$$
\begin{align*}
A\Sigma A^T=\Lambda \Lambda^T + \Omega-\Psi
\end{align*}
$$

and so the $A\Sigma A^T$ is perfectly estimated by the factor loadings,
up to the diagonal elements ($\Sigma, \Psi$ are diagonal). We could
analyse that

$$
\begin{align*}
A^{(2)}
&=(\delta I + (\Omega_{1,2} - \delta \Omega)\Sigma_{}^{-1})(\delta I + (\Omega_{1,2} - \delta \Omega)\Sigma_{}^{-1})^T\\
&=(\delta I + A-\delta I)(\delta I + A^T-\delta I)\\
&=AA^T
\end{align*}
$$

Otherwise, the analysis seems to get complex.
